...
class SendNewsletterTask extends AbstractTask
{
    ...
    const EXTENSION_NAME = 'Usertools';
    ...
    public function execute()
    {
        ...
        if ($this->newsletterLogRepository->countAll() == 0) {
            ...
            $nlqb = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_usertools_domain_model_newsletterlog');
            $nlqb->select('news')
            ...
            foreach ($this->newsRepository->createQuery()
                ...
                if ($n->getDatetime()->getTimestamp() < $plusOneHour && ($n->getArchive() == null || $n->getArchive()->getTimestamp() > $t)) {
                    ...
                    $subject = LocalizationUtility::translate('task.sendNewsletter.rememberEmail.subject', SendNewsletterTask::EXTENSION_NAME) . $n->getTitle();
                    ...
                    foreach ($uq->matching($uq->logicalAnd([
                        ...
                        ->execute() as $u) {
                            ...
                            MailUtility::getSystemFromAddress() => 'Newsletter-Service: '
                        ],[], $subject, 'NewsRememberEmail', SendNewsletterTask::EXTENSION_NAME, [
                            'targetUrl' => $this->newsDisplayUrl,
                            'user' => $u,
                            'news' => $n,
    ...
    public function getAdditionalInformation()
    {
        return 'News page id :' . $this->newsStoragePageId . ' / feUserPageId: ' . $this->feUserPageId . ' / logStoragePageId: ' . $this->logStoragePageId . ' / newsDisplayUrl: ' . $this->newsDisplayUrl . ' / maxCharacters: ' . $this->maxCharacters;
    }
